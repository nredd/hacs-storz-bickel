...
class CraftyDevice(SBDevice):
    ...
    async def _read_static_info(self) -> None:
        ...
        self._state.firmware_version = await self._try_read_string(c.CRAFTY_UUID_FIRMWARE_VERSION)
        self._state.ble_firmware_version = await self._try_read_string(c.CRAFTY_UUID_BLE_FIRMWARE_VERSION)
    ...
    def _handle_temperature(self, _char: BleakGATTCharacteristic, data: bytearray) -> None:
        """Decode the current chamber temperature (uint16 LE tenths)."""
        if len(data) < 2:
            return
        self._state.current_temperature = int.from_bytes(data[:2], byteorder="little") / c.TEMP_SCALE
    ...
    async def async_poll(self) -> SBDeviceState:
        ...
        self._state.led_brightness = await self._try_read_uint16(c.CRAFTY_UUID_LED_BRIGHTNESS)
        self._state.hours_of_operation = await self._try_read_uint16(c.CRAFTY_UUID_USE_HOURS)
        self._state.minutes_of_operation = await self._try_read_uint16(c.CRAFTY_UUID_USE_MINUTES)
    ...
    async def async_set_target_temperature(self, celsius: float) -> None:
        ...
        await self._write(c.CRAFTY_UUID_TARGET_TEMP, (safe * c.TEMP_SCALE).to_bytes(2, byteorder="little"))
    ...
    async def async_set_heater(self, *, on: bool) -> None:
        ...
        await self._write(c.CRAFTY_UUID_HEAT_ON if on else c.CRAFTY_UUID_HEAT_OFF, b"\x01")
    ...
    async def async_set_boost_temperature(self, celsius: float) -> None:
        ...
        await self._write(c.CRAFTY_UUID_BOOST_TEMP, (value * c.TEMP_SCALE).to_bytes(2, byteorder="little"))
    ...
    async def async_set_led_brightness(self, percent: int) -> None:
        ...
        await self._write(c.CRAFTY_UUID_LED_BRIGHTNESS, clamped.to_bytes(2, byteorder="little"))
    ...
    async def async_set_auto_shutoff_minutes(self, minutes: int) -> None:
        ...
        await self._write(c.CRAFTY_UUID_AUTO_OFF_COUNTDOWN, seconds.to_bytes(2, byteorder="little"))
