...
This document describes the technical architecture of the Storz & Bickel custom component for Home Assistant.

## Directory Structure

```text
custom_components/storz_bickel/
...
│   ├── __init__.py          # Exports StorzBickelDataUpdateCoordinator
...
│   ├── __init__.py          # Exports StorzBickelEntity
...
- `base.py` - Main coordinator class (`StorzBickelDataUpdateCoordinator`)
...
**Key class:** `StorzBickelDataUpdateCoordinator` (exported from `coordinator/__init__.py`)
...
**Key class:** `StorzBickelApiClient`
...
- `StorzBickelConfigFlowHandler` (main flow)
- `StorzBickelOptionsFlow` (options)
...
**Key class:** `StorzBickelEntity` (in `entity/base.py`)
...
2. `StorzBickelEntity` for common functionality
...
1. Create directory: `custom_components/storz_bickel/<platform>/`
2. Implement `__init__.py` with `async_setup_entry()`
3. Create entity classes inheriting from platform base + `StorzBickelEntity`
