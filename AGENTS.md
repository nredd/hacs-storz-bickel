...
- `custom_components/storz_bickel/` - Main integration code
...
- Integration logs: `custom_components.storz_bickel: debug` in `config/configuration.yaml`
...
- Use the domain `storz_bickel` for all DOMAIN references
- Prefix all integration-specific classes with `StorzBickel`
- Use "Storz & Bickel" as the display title
...
- Inherit from platform base + `StorzBickelEntity`
...
- `tests/` mirrors `custom_components/storz_bickel/` structure
